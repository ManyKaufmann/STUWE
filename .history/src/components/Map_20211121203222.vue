<template>
  <div>
    <div class="mapContainer">
      ...
      <div id="features">
        <div
          v-bind:id="'departement ' + (index + 1)"
          v-for="c in departements"
          :key="c.fields.title"
        >
          <Departement
            :departementName="station"
...
export default {
  ...
  methods: {
    ...
    //todo set class active when scrolling over section
    isElementOnScreen() {
    ...
    ifScrolling() {
      for (let chapterName in this.chapters) {
  ...
  created() {
    window.addEventListener("scroll", this.ifScrolling);
  },
  unmounted() {
    window.removeEventListener("scroll", this.ifScrolling);
  },
